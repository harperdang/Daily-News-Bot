...
def fetch_news():
    ...
    for media in MEDIA_LIST:
        try:
            ...
            if not feed.entries:
                print(f"  -> {media['name']} has no content.")
                continue
...
def send_email(content):
    ...
    # 邮件头里的 'To' 只显示为 "Global Readers"，保护隐私，不显示长长的一串名单
    ...
    try:
        ...
        with smtplib.SMTP_SSL(smtp_server, 465, context=context) as server:
            ...
            # 这里传入的是列表 receivers，SMTP 会自动群发给列表里的所有人
